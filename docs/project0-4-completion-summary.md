...
**错误信息**:
```
fatal: unable to access 'https://github.com/wjb412530/Orbit-Agent.git/': 
Failed to connect to github.com port 443 after 21153 ms: Could not connect to server
```

**解决方案**: 使用 GitHub CLI (gh) 推送
...
**问题**: CI 后端冒烟测试失败，无法导入 `app.api.server`

**错误信息**:
```
tavily.errors.MissingAPIKeyError: No API key provided. 
Please provide the api_key attribute or set the TAVILY_API_KEY environment variable.
```

**根本原因**: 
- `app/tools/tavily_tool.py` 在模块级别初始化 TavilyClient
- CI 环境缺少 TAVILY_API_KEY 环境变量
- 导入时触发 API key 检查导致失败

**解决方案**: 为 TavilyClient 提供默认值

```python
# 修改 app/tools/tavily_tool.py:23
tavily_client = TavilyClient(api_key=os.getenv("TAVILY_API_KEY", "dummy-key-for-ci"))
```

**提交信息**:
```
fix: 添加 Tavily API key 默认值以通过 CI 导入检查

CI 环境下缺少 TAVILY_API_KEY 导致模块导入失败,添加默认值 dummy-key-for-ci
使 CI 冒烟测试可以正常导入模块,实际 API 调用仍需有效密钥
...
1. **项目 5: 并发治理** - Semaphore 信号量限流
2. **项目 7: 评测体系** - 建立量化度量标准
3. **项目 8: 一键部署** - 完善 Docker Compose
